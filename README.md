...
**Sketches stay rigid inside a movable gear.** Cylindrical tooth sketches and shaft references are constrained in their component's local coordinates. The outer gear component remains ungrounded: choose **Move/Copy > Components** to move its body and sketches together. To upgrade an existing construction-history gear, install 0.3.2, fully restart Fusion, select the gear and use **Edit > Update gear** once. Generated curves are managed by Gear Studio; edit its inputs instead of removing their constraints.
