...
- [ ] Change `shaftDiameter` in Fusion's Parameters dialog. Confirm the add-in identifies changed inputs on refresh/edit. For gears created/updated in 0.3.2 with an existing bore, that diameter changes live; finish with **Update from Parameters** to validate and save the complete definition. Other geometry still requires Update.
...
- [ ] Complete-gear changes require explicit Update from Parameters (0.3.2 existing bores have a live diameter binding). Update replaces only generated Construction, preserving independent sketches outside it.
...
## 0.3.2 sketch constraints and live bore regression checks

User report: herringbone creation succeeds, but bore diameter/reference line freedom remains. The exact installed version was not supplied. All checks below concern the new constraints and remain pending in native Fusion.

- [ ] Create default external Spur, Helical and Herringbone. Edit Shaft bore cutter profile: center coincident to local origin, driving diameter expression uses its saved `Bore_Gn`, sketch reports fully constrained, and circle cannot translate or resize by dragging. Edit Pitch circle and shaft axis: both shaft endpoints are fixed, the line is construction geometry, pitch circle is centered/diameter-constrained. Tooth groove profile reports fully constrained and no spline points or tangent handles can drag.
- [ ] Create default internal cylindrical families: ring blank profile is centered and diameter-constrained. Repeat a crown backing web and bores in the other rotary families to check the shared helper. No unrelated family acceptance is implied by a cylindrical pass.
- [ ] Change an existing bore from 5 mm to 6 mm through its Fusion user parameter and measure the hole before invoking Update. Confirm it follows the expression with the add-in stopped as well. Restart Gear Studio, refresh selection and confirm pending inputs are still marked stale; Update from Parameters validates and records the new definition.
- [ ] In the panel, switch bore from 5 mm to 0 mm and back. Update succeeds without a zero-radius sketch; changes from/to no bore require this regeneration. Repeat coordinated module 1 to 2 mm and bore 5 to 30 mm on a default 24-tooth helical gear: old construction must not break before replacement, and the new bore remains bound to the same alias.
- [ ] In a disposable design, attempt an impossible bore directly in Fusion's table. Observe the native solver result, Undo/restore the valid value, then confirm Gear Studio can update. Repeat the invalid input in the Gear Studio panel: validation must block building before any geometry or parameter change.
- [ ] Move/rotate the outer component, capture its position, update the gear, then save/reopen. Body and sketches remain aligned and placement remains unchanged. Generated child occurrences stay grounded to parent; outer occurrence stays movable. Sketch constraints must not ground it to the document origin.
- [ ] Update a pre-0.3.2 native-history gear without changing input values. The same outer component and parameter names remain, and its new sketches gain constraints. Merely installing the new code does not alter saved geometry. Duplicate and legacy name-shortening paths preserve independent and valid diameter references.
- [ ] Undo/Redo successful creation and update; inspect both the bore expression and measured hole. Induce a staged update failure with an external dependency in a disposable design: the native command must restore the original tagged diameter expression, parameters, construction and placement, with no orphan subtree.
